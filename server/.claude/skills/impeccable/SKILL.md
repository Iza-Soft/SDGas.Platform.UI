...
## Project defaults (personalized for this project)

This install is tuned for a solo/small-team **React/Next.js product build**, prioritizing accessibility, performance, and pre-ship polish over pure visual experimentation:

- **Default priorities.** When routing with no argument, or when a request is ambiguous about which command fits, prefer `audit`, `harden`, `optimize`, and `polish` over `bolder`, `overdrive`, or `delight`. This only governs *unprompted* suggestions — if the user explicitly asks for something bolder, louder, or more experimental, follow that request exactly as normal; nothing here restricts what commands are available.
- **Stack context.** This project builds on React/Next.js. Wherever guidance is stack-specific, favor Next.js-native patterns: `next/image` and `next/font` for performance work (`optimize`, `audit`), hydration-safe motion for `animate` (watch for server/client render mismatches), Server vs. Client Component boundaries when suggesting structural changes, and accessible patterns for common React headless-UI primitives (Radix, Headless UI, React Aria) during `audit`/`harden`.
- **No standalone shortcuts.** Every command is invoked as `/impeccable <command>` (e.g. `/impeccable audit`, `/impeccable polish`) — never a bare `/audit` or `/polish`. Do not run `pin.mjs` to create standalone shortcuts for this project unless the user explicitly asks for one later.
- **Hook already on.** The design detector hook ships pre-wired and enabled via the committed `.claude/settings.json` and the default `hook.enabled: true` — no need to run `/impeccable hooks on`. `/impeccable hooks off` still works if the user wants to disable it.
