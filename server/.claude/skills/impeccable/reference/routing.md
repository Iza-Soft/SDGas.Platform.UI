...
- Otherwise group by intent (build new / improve what's there / iterate visually), tailored to the current surface and `setup.platform`. This project's default priorities are accessibility, performance, and pre-ship polish (see SKILL.md's "Project defaults" section) — when signals don't clearly point elsewhere, lead with `audit`, `harden`, `optimize`, or `polish` rather than `bolder`, `overdrive`, or `delight`.
